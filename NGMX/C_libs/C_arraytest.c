...
static PyMethodDef _C_arraytestMethods[] = {
	{"C_arrayinfo", C_arrayinfo, METH_VARARGS},
...
static PyObject *C_arrayinfo(PyObject *self, PyObject *args)
{
	int i;
	float *ptr;
	PyArrayObject *arr;
	...
	printf("Array has %i Dimensions:\n", PyArray_NDIM(arr) );
	for(i=0; i<PyArray_NDIM(arr); i++)
	{
		printf("Dimension %i with a shape %i and a stride %i \n", i, PyArray_DIM(arr, i), PyArray_STRIDE( arr, i) );
	}
	printf("with arraytype %i\n",PyArray_TYPE(arr));
