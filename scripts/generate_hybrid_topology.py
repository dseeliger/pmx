    ...
    for b in topol.bonds:
        ...
        if a1.atomtypeB is not None or a2.atomtypeB is not None:
            ...
            # catch errors
                ...
                sys.exit(1)
            if astate == None:
                logger.error('Error A: No bond entry found for astate %s-%s (%s-%s -> %s-%s)' % (a1.name,a2.name,a1.atomtype,a2.atomtype, a1.type,a2.type))
                logger.error('Resi = %s' % a1.resname)
                error_occured = True

            if bstate == None:
                logger.error('Error B: No bond entry found for bstate %s-%s (%s-%s -> %s-%s)' % (a1.name,a2.name,a1.atomtypeB,a2.atomtypeB, a1.typeB,a2.typeB))
                error_occured = True
            if error_occured:
                sys.exit(1)
    logger.info('Making bonds for state B -> %d bonds with perturbed atoms' % count)
...
#		    print '%s %s %s' % (a1.name, a2.name, a3.name)
    ...
    logger.info('Making angles for state B -> %d angles with perturbed atoms' % count)
...
#                               print '%s' %d
    ...
    logger.info('Making dihedrals for state B -> %d dihedrals with perturbed atoms' % count)
    logger.info('Removed %d fake dihedrals' % nfake)
...
def get_hybrid_residue(residue_name, mtp_file = 'ffamber99sb.mtp', version = 'old'):
    logger.info('Scanning database for %s ' % residue_name)
    resi, bonds, imps, diheds, rotdic = read_mtp_entry(residue_name, filename = mtp_file, version = version)
    if len(resi.atoms) == 0:
        logger.error("Hybrid residue %s not found in %s" % (residue_name, mtp_file) )
        sys.exit(1)
...
def get_ff_path( ff ):
    ...
    if not os.path.isdir(ff):
        ...
        if os.path.isdir(p):
            ...
            logger.error(' Error: forcefield path "%s" not found' % ff)
            sys.exit(1)
    else:
        ff_path = ff
    logger.info('Opening forcefield: %s' % ff_path)
    return ff_path
...
def main(argv):
    ...
    if input_itp and out_file.split('.')[-1] != 'itp':
        ...
        logger.info('Setting outfile name to %s' % out_file)
...
#        topol = Topology( input_itp,  topfile = top_file, version = 'new', ff = cmdl['-ff'] )
        logger.info('Reading input .itp file "%s""' % (input_itp))
        topol = Topology( input_itp,  topfile = None, version = 'new', ff = cmdl['-ff'], ffpath=get_ff_path(cmdl['-ff']) )
    else:
        logger.info('Reading input .top file "%s"' % (top_file))
...
#	    print atom.typeB
    for r in rlist:
        logger.info('Hybrid Residue -> %d | %s ' % (r.id, r.resname ))
    ...
    logger.info('Total charge of state A = %4.2f' %topol.get_qA())
    logger.info('Total charge of state B = %4.2f'% topol.get_qB())
...
#    topol.check_special_dihedrals( )
    ...
    if cmdl['-split']: # write splitted topology
        ...
        logger.info('------------------------------------------------------')
        logger.info('Creating splitted topologies............')
        logger.info('Making "qoff" topology : "%s"' % out_file_qoff)
        ...
        logger.info('Charge of state A: %g' % topol.qA)
        logger.info('Charge of state B: %g' % topol.qB)

        logger.info('------------------------------------------------------')
        logger.info('Making "vdw" topology : "%s"' % out_file_vdw)
        contQ = copy.deepcopy(qA_mem)
        ...
        logger.info('Making "qon" topology : "%s"' % out_file_qon)
    ...
    logger.info('making b-states done...........')
